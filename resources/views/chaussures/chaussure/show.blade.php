...
@section('content')
                    ...
                    <form action="{{ route('panier_add', ['id' => $chaussure->id_chaussure]) }}" method="post"
                        ...
                        <div class="lg:w-5/5 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0" style="cursor: auto;">
                            ...
                            <h1 class="text-gray-900 text-3xl title-font font-medium mb-1" style="cursor: auto;">
                                {{ $chaussure->modele }}</h1>
                            <h1 class="text-gray-900 py-2-" style="cursor: auto;">Chaussures pour {{ $chaussure->genre }}
                            </h1>
                            @if ($pourcentage)
                                                    ...
                                                    <div class="overflow-y-scroll max-h-80">
                                                        ...
                                                        @if (count($avis) > 0)
                                                            @foreach ($avis as $avisItem)
                                                                <section
                                                                    class="relative flex min-w-screen border-b-slate-600">
                                                                    <div
                                                                        class="container px-0 mx-auto sm:px-5 py-2 border-b-slate-600">
                                                                        <div
                                                                            class=" py-4 mx-auto bg-white border-b-slate-600 border-b-2  sm:px-4 sm:py-4 md:px-4  sm:shadow-sm md:w-3/3">
                                                                            <div class="flex flex-col w-full">
                                                                                <div
                                                                                    class="flex flex-row w-full justify-between items-center">
                                                                                    <div class="flex flex-col mt-1">
                                                                                        <div
                                                                                            class="flex items-center flex-1 px-4 font-bold leading-tight text-gray-700">
                                                                                            {{ $avisItem->user->nom . ' ' . $avisItem->user->prenom }}
                                                                                        </div>
                                                                                        <div
                                                                                            class="flex items-center flex-1 px-4 font-normal text-xs text-gray-500">
                                                                                            {{ $avisItem->created_at->format('d/m/Y H:i') }}
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                                                                                <div
                                                                                    class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600">
                                                                                    {{ $avisItem->commentaire }}</div>
                                                            ...
                                                            @endforeach
                                                        @else
                                                            <p class="">Aucun commentaire n'a encore été fait pour
                                                                cette chaussure.</p>
                                                        @endif
                                                    </div>
                                                </div>
                                ...
                                <div class="flex ml-6 items-center">
                                    ...
                                    <div class="relative">
                                        <select name="taille" id="taille"
                                            class="rounded border border-gray-300 py-2 px-3 focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 text-base"
                                            required>
                            ...
                            <div class="w-full flex flex-col">
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea id="message" name="message"
                                        class="w-full px-4 py-2 mt-2 text-base text-gray-700 bg-gray-100 border  rounded-lg focus:outline-none focus:border-gray-500 @error('message') border-red-500 @enderror"
                                ...
                                <div class="mt-4">
                                    ...
                                    <x-text-input id="captcha_input" class="block mt-1 w-full" type="text"
                                        name="captcha" required />
                                    <x-input-error :messages="$errors->get('captcha')" class="mt-2" />
                                </div>
                                <button
                                    class="rounded border-0 bg-indigo-500 py-2 px-6 text-lg text-white hover:bg-indigo-600 focus:outline-none">Poster</button>
                ...
                @else
                    <div class="max-w-xl mx-auto mt-16 flex w-full flex-col rounded-lg bg-white p-8">
                        ...
                        <form action="{{ route('login') }}">
                            <button type="submit"
                                class="rounded border-0 bg-indigo-500 py-2 px-6 text-lg text-white hover:bg-indigo-600 focus:outline-none justify-center">Se
                                connecter</button>
                        </form>
    ...
    <script>
        ...
        // Sélectionner l'élément de message d'erreur
        ...
        stars.forEach((star) => {
            star.addEventListener('click', () => {
                ...
                updateRating();
                // Vérifier si une étoile a été sélectionnée
                validateRating();
            });
            ...
            star.addEventListener('mouseover', () => {
                ...
                const prevStars = Array.from(star.parentNode.children).slice(0, star.getAttribute(
                    'data-value') - 1);
        ...
        // Fonction pour valider la sélection d'une étoile
