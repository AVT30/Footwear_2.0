...
@section('content')
    <div class="p-10 grid grid-cols-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-5">
        ...
        <div class="max-w-xl py-5">
            <h1 class="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">Sneakers basses</h1>
            <p class="mt-2 text-sm text-gray-500"> Pas de chaussures "Sneakers basses" pour l'instant</p>
        </div>
        @endif
                            ...
                            <div class="p-4 bg-white shadow w-80">
                                <div class="flex justify-center relative rounded-lg overflow-hidden h-96 w-72">
                                    ...
                                    <div class="absolute flex flex-col top-0 right-0 p-3">
                                        ...
                                        @if ($chaussure->wishlists()->where('id_utilisateur', Auth::id())->exists())
                                            <a href="{{ route('whislist.supprimerwhislist', $chaussure->id_chaussure) }}"
                                                class="opacity-70 bg-red hover:opacity-100 cursor-pointer hover:text-red-700 shadow hover:shadow-md text-red-700 rounded-full w-8 h-8 text-center p-1">
                                    ...
                                    @if ($chaussure->pourcentage)
                                        <span
                                            class="absolute bottom-0 left-0 inline-flex mb-3 ml-3 px-3 py-2 rounded-lg z-10 bg-red-600 text-sm font-medium text-white select-none">
                                ...
                                <div class="grid grid-cols-1">
                                    <div class="flex items-center justify-start">
                                        <p
                                            ...
                                            @if ($chaussure->pourcentage)
                                                <span class="text-lg text-red-600">{{ number_format($chaussure->prix, 2) }}
                                                    CHF <span
                                                        class="line-through text-gray-500 ">{{ number_format($chaussure->prix / (1 - $chaussure->pourcentage / 100), 2) }}
                                                        CHF</span></span>
                                            @else
                                                <span class="text-lg">{{ number_format($chaussure->prix, 2) }}
                                                    CHF</span>
                                            @endif
                                        </p>
                                    </div>
        ...
        @if ($chaussuresPagineesParType['Sneakers basses']->count() > 0)
            <div class="col-span-12 flex justify-center py-4">
                {{ $chaussuresPagineesParType['Sneakers basses']->links() }}
            </div>
        @endif
