    ...
    <div class="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:pb-24 lg:px-8">
      ...
      <div class="mt-16">
          ...
          <div class="space-y-20">
              <div>
                  <div class="bg-gray-200 rounded-lg py-6 px-4 sm:px-6 sm:flex sm:items-center sm:justify-between sm:space-x-6 lg:space-x-8">
                      ...
                      <a href="#" class="w-full flex items-center justify-center bg-white mt-6 py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:w-auto sm:mt-0" onclick="window.print()">
                        Imprimer commande
                  ...
                  <table class="mt-4 w-full text-gray-500 sm:mt-6">
                      ...
                      <thead class="sr-only text-sm text-gray-500 text-left sm:not-sr-only">
                          <tr>
                              ...
                              <th scope="col" class="hidden w-1/5 pr-8 py-3 font-normal sm:table-cell">Prix</th>
                          ...
                          @foreach ($commandes as $commande)
                            ...
                            <td class="py-6 pr-8">
                                <div class="flex items-center">
                                    <img src="{{ asset('storage/images/'.$commande->chaussure->image->image_chaussure) }}" alt="{{ $commande->chaussure->modele }}" class="w-16 h-16 object-center object-cover rounded mr-6">
                                    <div>
                                        <div class="font-medium text-gray-900">{{ $commande->chaussure->modele }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            @if ($rabais = Rabais::where('id_chaussure', $commande->chaussure->id_chaussure)->first())
                                <td class="hidden py-6 pr-8 sm:table-cell">{{ number_format($commande->chaussure->prix - ($commande->chaussure->prix * $rabais->rabais / 100), 2) }} CHF</td>
                                @else
