                ...
                @foreach($avisusers as $avis)
                        ...
                        <div class="flex items-center gap-4">
                            @if($avis->user->genre == "homme")
                                <img class="h-16 w-16 rounded-full object-cover" src="storage/images/man.png" alt="Avatar">
                            @else
                                <img class="h-16 w-16 rounded-full object-cover" src="storage/images/woman.png" alt="Avatar">
                            @endif
                            <div>
                                ...
                                <p class="mt-1 text-lg font-medium text-gray-700">{{$avis->user->nom . ' ' . $avis->user->prenom }}</p>
                                <p class="mt-1 text-sm font-light text-gray-400">{{$avis->created_at->format('d/m/Y H:i')}}</p>
                            </div>
                        </div>
                        <p class="line-clamp-2 sm:line-clamp-none mt-4 text-gray-500"> {{$avis->commentaire}} </p>
                            <div class="flex items-center gap-x-4">
                                <form action="{{ route('avis.accepter', $avis->id_avis) }}" method="POST" onsubmit="scrollToSection('gerer-avis')">
                                    @csrf
                                    <button type="submit" class="text-green-500 hover:text-green-700 focus:outline-none">Accepter</button>
                                </form>

                                <form action="{{ route('avis.supprimer', $avis->id_avis) }}" method="POST" onsubmit="scrollToSection('gerer-avis')">
                                    @csrf
                                    @method('DELETE')
